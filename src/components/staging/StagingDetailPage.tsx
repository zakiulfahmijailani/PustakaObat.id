...
import { ArrowLeft, ArrowUpRight, Clock3, FileWarning, FlaskConical, ShieldAlert } from 'lucide-react'
...
export async function StagingDetailPage({ drugKey, basePath }: { drugKey: string; basePath: string }) {
  ...
  const { concept, evidence, sources, drafts, events, error } = await getStagedDrugForStaff(drugKey)
  ...
  return (
    <div className="space-y-8">
      ...
      <Card className="border-primary/20"><CardHeader><CardTitle>Editorial Bahasa Indonesia</CardTitle></CardHeader><CardContent><StagingEditorialForm drugKey={concept.drug_key} isPilot={concept.is_pilot} availableSections={evidenceBySection.map(([section]) => section)} drafts={drafts} /></CardContent></Card>
