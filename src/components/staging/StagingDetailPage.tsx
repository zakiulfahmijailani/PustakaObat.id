...
import { ArrowLeft, ArrowUpRight, BadgeCheck, BookOpenText, Clock3, FlaskConical, ShieldAlert } from 'lucide-react'
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card><CardHeader><CardTitle>Cakupan</CardTitle></CardHeader><CardContent className="space-y-2 text-sm text-text-muted"><p>{evidenceBySection.length} bagian evidence</p><p>{evidence.length} record evidence</p><p>{sources.length} dokumen sumber</p></CardContent></Card>
      </div>
      ...
      <section className="space-y-4"><div><h2 className="font-serif text-3xl text-text">Sources staging lama</h2><p className="mt-1 text-sm text-text-muted">Jejak sumber historis. Untuk draf baru, pembanding resmi Reviewer adalah seksi FDA yang terikat pada draf, bukan panel ini.</p></div>{evidenceBySection.map(([section, records]) => <Card key={section}><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle>{SECTION_LABELS[section] || section}</CardTitle><div className="flex flex-wrap items-center gap-2"><Badge variant="outline">Untuk obat: {concept.preferred_name}</Badge><Badge variant="outline">{records?.length || 0} sumber</Badge></div></div></CardHeader><CardContent className="space-y-4">{records?.map((item) => <details key={item.evidence_id} className="rounded-2xl border border-border bg-surface-2 p-5"><summary className="cursor-pointer font-bold text-text">{concept.preferred_name} · {item.source_name} · {item.source_section}</summary><div className="mt-4 flex flex-wrap gap-2"><Badge variant="outline">Kecocokan sumber: {item.ingredient_match_status}</Badge>{item.product_specific && <Badge variant="warning">Spesifik produk</Badge>}<Badge variant="destructive">Tidak untuk publik</Badge></div><p className="mt-4 whitespace-pre-wrap text-sm leading-relaxed text-text-muted">{item.source_text}</p><p className="mt-4 text-xs text-text-muted">Evidence ID {item.evidence_id} · publication_eligible=false</p></details>)}</CardContent></Card>)}</section>

      <SourceDocumentsPanel sources={sources} />
