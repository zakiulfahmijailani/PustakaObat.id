...
export function BoundEvidencePanel({
  ...
  return <section className="space-y-3 rounded-2xl border border-primary/20 bg-primary/5 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><BookOpenText className="text-primary" size={20} /><p className="font-bold text-text">Evidence FDA terikat · {drugName}</p></div><Badge variant="outline">{sections.length} seksi sumber</Badge></div><p className="text-sm leading-relaxed text-text-muted">Teks ini adalah bahan pembanding internal yang sama untuk Editor dan Reviewer. Tidak diterbitkan langsung. {translationOverlayState === 'available' ? 'Draf Indonesia AI ditampilkan sebagai pembanding dan wajib direview apoteker.' : translationOverlayState === 'unavailable' ? 'Draf Indonesia privat sedang tidak dapat dimuat.' : ''}</p>{sections.map((section) => <details key={section.section_id} open className="rounded-xl border border-border bg-surface p-4"><summary className="cursor-pointer font-bold text-text">{sourceTitle(section)}</summary><div className="mt-3 space-y-4"><div><p className="text-xs font-bold uppercase tracking-wider text-text-muted">Sumber Inggris</p><p className="mt-2 whitespace-pre-wrap break-words text-sm leading-7 text-text-muted">{section.source_text || 'Teks sumber tidak tersedia.'}</p></div>{section.indonesian_draft && <div className="rounded-xl border border-primary/20 bg-primary/5 p-4"><p className="text-xs font-bold uppercase tracking-wider text-primary">Draf Indonesia AI · perlu review apoteker</p><p className="mt-2 whitespace-pre-wrap break-words text-sm leading-7 text-text">{section.indonesian_draft}</p></div>}<div className="flex flex-wrap gap-2 text-xs text-text-muted"><span>FDA field: {section.section_type} · {section.source_character_count.toLocaleString('id-ID')} karakter</span>{section.translation_status === 'AI_TRANSLATED_UNREVIEWED' && <Badge variant="warning">AI translated · unreviewed</Badge>}{section.translation_quality_flags_json !== '[]' && <Badge variant="warning">Perlu cek QC otomatis</Badge>}</div></div></details>)}</section>
}
