...
export function EditorialReviewPanel({ drafts, aiCandidates = [], actorId, drugName, initialSection }: { drafts: EditorialDraft[]; aiCandidates?: IndonesianCandidateDraft[]; actorId: string; drugName: string; initialSection?: string }) {
  ...
  return <div className="space-y-6">
    ...
    {draft ? <><div className="rounded-2xl border border-border bg-surface-2/40 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><p className="font-bold text-text">Draf Bahasa Indonesia</p><span className="text-xs font-bold uppercase tracking-wider text-text-muted">{draft.status.replaceAll('_', ' ')}</span></div><RichTextContent value={draft.content_indonesian} className="mt-4 text-sm text-text" /></div>{hasBoundEvidence ? <BoundEvidencePanel labelId={draft.source_label_id!} sectionTypes={draft.source_section_types} drugName={drugName} /> : <div className="rounded-2xl border border-warning/30 bg-warning/5 p-5 text-sm text-text-muted"><CircleAlert className="mb-2 text-warning" size={19} /><strong className="text-text">Draf lama belum memiliki evidence FDA terikat.</strong> Bagian ini tidak boleh disetujui. Gunakan tombol minta revisi agar Editor dapat mengikat label dan seksi sumber yang sesuai.</div>}{canRequestChanges ? <div className="space-y-4 rounded-2xl border border-primary/20 bg-primary/5 p-5"><Textarea label="Catatan untuk Editor" value={note} onChange={(event) => setNote(event.target.value)} disabled={pending} helperText={hasBoundEvidence ? 'Catatan wajib jika meminta revisi.' : 'Jelaskan bahwa Editor harus mengikat evidence FDA sebelum mengirim ulang.'} /><div className="flex flex-wrap gap-3">{canApprove && <Button type="button" disabled={pending} onClick={() => review('approve')}><CheckCircle2 size={17} />Setujui bagian</Button>}<Button type="button" variant="outline" disabled={pending || !note.trim()} onClick={() => review('changes_requested')}><Undo2 size={17} />Minta revisi</Button></div></div> : <div className="rounded-2xl bg-surface-2 p-5 text-sm text-text-muted">{draft.status === 'submitted' ? 'Draf ini ditulis oleh akun Anda sehingga harus diperiksa Reviewer lain.' : draft.reviewer_note || 'Belum ada tindakan review yang diperlukan.'}</div>}</> : <div className="rounded-2xl border border-dashed border-border p-6 text-sm text-text-muted">Bagian ini masih berupa kandidat AI dan belum menjadi draf yang dapat disetujui.</div>}
