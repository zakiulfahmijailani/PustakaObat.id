...
import type { EditorialDraft, EditorialEvent, StagingDrugConcept, StagingEvidence, StagingSourceDocument } from './types'
...
export async function getStagedDrugForStaff(drugKey: string) {
  if (!isNeonConfigured()) return { concept: null, evidence: [], sources: [], drafts: [], events: [], error: new Error('Neon is not configured.') }
  try {
    const [conceptRows, evidence, sources, drafts, events] = await Promise.all([
    ...
    return { concept: conceptRows[0] || null, evidence, sources, drafts, events, error: null }
  } catch (error) {
    return { concept: null, evidence: [] as StagingEvidence[], sources: [] as StagingSourceDocument[], drafts: [] as EditorialDraft[], events: [] as EditorialEvent[], error }
  }
}
