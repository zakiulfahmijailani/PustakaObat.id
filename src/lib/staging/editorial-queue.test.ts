...
describe('actionable editorial queues', () => {
  ...
  it('excludes claimed work and self-review from role queues', async () => {
    ...
    expect(editorQueue).toContain('unnest(ready.available_section_types)')
    expect(editorQueue).not.toContain('monograph_staging_indonesian_drafts ai')
  ...
  it('derives every new Editor section from translated bilingual evidence', async () => {
    const migration = await readFile(resolve('database/migrations/020_editorial_full_label_sections.sql'), 'utf8')
    const sync = await readFile(resolve('scripts/full-label-v3.2/sync-editorial-availability.mjs'), 'utf8')
    const mapping = JSON.parse(await readFile(resolve('src/lib/full-label/section-mapping.json'), 'utf8'))
    expect(migration).toContain('available_section_types text[]')
    expect(sync).toContain('section.indonesian_draft?.trim()')
    expect(sync).toContain('availableSectionTypes')
