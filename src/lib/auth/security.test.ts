...
describe('Neon authentication security helpers', () => {
  ...
  it('routes account states without trusting a client role', () => {
    ...
    expect(getSafeRedirectForAccount('active', 'editor')).toBe('/editor/dashboard')
    expect(getSafeRedirectForAccount('active', 'super_admin')).toBe('/super-admin/choose-role')
  ...
  it('uses auth intent only to route unlinked Google identities', () => {
    ...
    expect(parseAuthIntent('editor_login')).toBe('editor_login')
    expect(parseAuthIntent('super_admin_login')).toBe('super_admin_login')
    ...
    expect(getUnlinkedAccountDestination('editor_login')).toBe('/editor/access-denied')
    expect(getUnlinkedAccountDestination('super_admin_login')).toBe('/super-admin/access-denied')
  ...
  it('keeps reviewer and admin entry points separate without trusting intent as role', () => {
    ...
    expect(authServer).toContain("requireActiveProfile(['reviewer'], '/reviewer/not-registered')")
    expect(authServer).toContain("requireActiveProfile(['admin'], '/admin/access-denied')")
    expect(authServer).toContain("requireActiveProfile(['editor'], '/editor/access-denied')")
    expect(authServer).toContain('WORKSPACE_COOKIE')
  ...
  it('preauthorizes admins without silently promoting an existing reviewer', () => {
    ...
    expect(adminUsers).toContain("session.activeRole !== 'admin'")
  ...
  it('keeps Admin Management server-controlled and reviewer-safe', () => {
    ...
    expect(adminManagement).toContain("session.activeRole !== 'admin'")
  ...
  it('keeps super-admin workspace selection server-controlled', () => {
    const workspaceRoute = readFileSync('src/app/api/auth/workspace/route.ts', 'utf8')
    const stagingRoute = readFileSync('src/app/api/staging/editorial/route.ts', 'utf8')
    expect(workspaceRoute).toContain("session.profile.role !== 'super_admin'")
    expect(workspaceRoute).toContain('httpOnly: true')
