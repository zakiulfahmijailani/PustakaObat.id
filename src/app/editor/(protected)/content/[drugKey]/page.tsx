...
export default async function EditorContentDetailPage({ params, searchParams }: { params: Promise<{ drugKey: string }>; searchParams: Promise<{ section?: string }> }) {
  ...
  const { concept, drafts, availableSections, error } = await getStagedDrugForEditor(drugKey, user.id)
