...
export default async function WhoReviewQueuePage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap gap-2">{statuses.map(([value, label]) => <Button key={value} size="sm" variant={status === value ? 'primary' : 'outline'} asChild><Link href={`/dashboard/who?status=${value}${filters.q ? `&q=${encodeURIComponent(filters.q)}` : ''}`}>{label}</Link></Button>)}</div>
      ...
      {error ? <p className="rounded-2xl bg-error/10 p-5 text-error">Katalog WHO belum tersedia. Pastikan migration dan import sudah dijalankan.</p> : medicines.length ? (
        <div className="space-y-4">{medicines.map((medicine) => (
          <Link key={medicine.id} href={`/dashboard/who/${medicine.id}`} className="flex flex-col gap-5 rounded-3xl border border-border bg-surface p-6 transition-colors hover:border-primary/30 md:flex-row md:items-center md:justify-between">
        ...
        ))}<CatalogPagination page={page} count={count} pathname="/dashboard/who" params={{ q: filters.q, status }} /></div>
