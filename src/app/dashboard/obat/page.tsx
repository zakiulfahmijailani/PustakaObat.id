  ...
  return (
    <div className="space-y-10">
      ...
      <Card className="border-none bg-surface-2/40"><CardContent className="flex flex-col gap-6 p-6 md:flex-row md:p-8">
        ...
        <div className="flex gap-2 overflow-x-auto">{statusOptions.map((option) => <Link key={option.value} href={`/admin/medicines?status=${option.value}${q ? `&q=${encodeURIComponent(q)}` : ''}`} className={`whitespace-nowrap rounded-full border px-4 py-2 text-xs font-bold uppercase tracking-wider ${(status || 'all') === option.value ? 'border-primary bg-primary text-white' : 'border-border text-text-muted'}`}>{option.label}</Link>)}</div>
      </CardContent></Card>
      ...
      {error ? <p className="rounded-2xl bg-error/10 p-5 text-error">Monografi tidak dapat dimuat: {error.message}</p> : drugs.length ? <div className="space-y-4">{drugs.map((drug) => <Card key={drug.id} className="border-none bg-surface-2/40 p-2"><CardContent className="flex flex-col justify-between gap-6 p-6 md:flex-row md:items-center">
        ...
        <div className="flex gap-2">{drug.status === 'published' && <Button variant="outline" size="sm" asChild><Link href={`/obat/${drug.slug}`} target="_blank"><Eye size={16} /> Publik</Link></Button>}<Button variant="outline" size="sm" asChild><Link href={`/admin/medicines/${drug.id}/edit`}><Edit3 size={16} /> Edit</Link></Button><ChevronRight className="self-center text-text-muted" size={20} /></div>
