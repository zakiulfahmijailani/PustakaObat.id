-- Apoteq WHO catalog for Neon PostgreSQL.
-- Run with the Neon SQL Editor or psql. This file contains no Supabase roles,
-- auth.uid(), RLS policy names, or service-role grants.
...
create table if not exists public.who_medicines (
  ...
  aware_category text check (aware_category in ('Access', 'Watch', 'Reserve', 'Not recommended')),
  is_monitoring_only boolean not null default false,
  is_not_on_eml boolean not null default false,
  data_status text not null check (data_status in ('WHO_ONLY', 'WHO_AND_AWARE', 'AWARE_ONLY')),
...
create unique index if not exists idx_who_medicines_external_id
  on public.who_medicines(external_id) where external_id is not null;
create index if not exists idx_who_medicines_normalized_name on public.who_medicines(normalized_name);
create index if not exists idx_who_medicines_aware_category
  on public.who_medicines(aware_category) where aware_category is not null;
create index if not exists idx_who_medicines_public_listing
  on public.who_medicines(is_active, publication_status, medicine_name);
create index if not exists idx_who_medicines_verification_status
  on public.who_medicines(verification_status);

drop trigger if exists set_who_medicines_updated_at on public.who_medicines;
create trigger set_who_medicines_updated_at
before update on public.who_medicines
for each row execute function public.set_updated_at();

create table if not exists public.who_medicine_verifications (
  id uuid primary key default gen_random_uuid(),
  ...
  status text not null check (status in ('verified', 'rejected', 'needs_revision')),
  note text,
...
create index if not exists idx_who_verifications_medicine_created
  on public.who_medicine_verifications(medicine_id, created_at desc);
create index if not exists idx_who_verifications_user
  on public.who_medicine_verifications(verified_by, created_at desc);

create table if not exists public.who_import_runs (
  id uuid primary key default gen_random_uuid(),
  source_name text not null default 'WHO' check (source_name = 'WHO'),
  source_version text not null,
...
create index if not exists idx_who_import_runs_imported_at
  on public.who_import_runs(imported_at desc);

create table if not exists public.who_audit_logs (
  id uuid primary key default gen_random_uuid(),
  actor_id uuid not null,
  action text not null,
  medicine_id uuid references public.who_medicines(id) on delete set null,
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);

create index if not exists idx_who_audit_logs_medicine_created
  on public.who_audit_logs(medicine_id, created_at desc);
...
begin
  ...
  insert into public.who_import_runs (
    ...
    total_count, inserted_count, updated_count, unchanged_count + coalesce(_skipped_count, 0),
...
begin
  if _actor_id is null then raise exception 'Reviewer is required'; end if;
  ...
  if previous_status is null then raise exception 'WHO medicine record not found'; end if;
  ...
  ) values (_record_id, previous_status, _decision, nullif(trim(_note), ''), _actor_id);

  insert into public.who_audit_logs (actor_id, action, medicine_id, metadata)
  values (
    _actor_id, 'REVIEW_WHO_MEDICINE', _record_id,
...
begin
  if _actor_id is null then raise exception 'Administrator is required'; end if;
  if _publication_status not in ('published', 'hidden') then raise exception 'Invalid publication status'; end if;

  select * into previous_record from public.who_medicines where id = _record_id for update;
  if previous_record.id is null then raise exception 'WHO medicine record not found'; end if;
  ...
  insert into public.who_audit_logs (actor_id, action, medicine_id, metadata)
  values (
    _actor_id, 'UPDATE_WHO_MEDICINE', _record_id,
    jsonb_build_object(
      'before', jsonb_build_object(
        'editorial_name', previous_record.editorial_name,
        'publication_status', previous_record.publication_status,
        'is_active', previous_record.is_active
      ),
      'after', jsonb_build_object(
        'editorial_name', updated_record.editorial_name,
        'publication_status', updated_record.publication_status,
